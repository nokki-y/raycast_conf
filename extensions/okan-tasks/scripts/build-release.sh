...
| **通知間隔** | 通知を送る間隔 | デフォルト: 1時間（推奨） |
| **通知音** | 通知時に鳴らす音 | デフォルト: Basso（低音） |
...
- Raycast再起動時は自動的にリセットされ、すぐに通知が来ます
...
4. 音が気に入らなければ、Configure Extension → 通知音 を変更して再テスト
...
- **通知音をカスタマイズ**可能（14種類から選択）
- **通知音テスト**機能で事前に音を確認できる
